...
/**
 * "Order Now" morphing blob button. 1:1 port of the original component
 * (module 23057 in _reference/_original/_next/static/chunks).
 */
export default function BlobButton() {
  ...
  return (
    <Link
      ...
      type="link-btn"
      onMouseEnter={() => {
        const shape = HOVER_PATHS[Math.floor(Math.random() * HOVER_PATHS.length)];
      ...
      onMouseLeave={() => {
        tweenRef.current?.kill();
        tweenRef.current = gsap.to(pathRef.current, {
          keyframes: [{ morphSVG: { shape: BASE_PATH }, duration: 0.5, ease: "back.in" }],
        });
      }}
