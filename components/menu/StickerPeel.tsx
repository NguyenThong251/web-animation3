...
/**
 * Peelable sticker (original module 80308). The peel amount is scrubbed via
 * ScrollTrigger (CSS var --peel-amount drives the clip-path), the SVG point
 * lights follow the cursor, and coarse-pointer/Safari devices fall back to
 * simple effects.
 */
export default function StickerPeel({
  ...
  useEffect(() => {
    ...
    if (coarse || isSafari) setSimpleEffects(true);
  }, []);
  ...
  return (
    <div className={`${className} ${simpleEffects ? "simple-effects" : ""}`} ref={rootRef} style={cssVars}>
