...
/**
 * "Order Now" morphing blob button (1:1 rebuild of the original site's
 * blob button — module 23057 in the original client bundle).
 *
 * Hover: morph to a random alternate blob, duration .5, ease "back.out".
 * Leave: morph back to the base blob, duration .5, ease "back.in".
 */
const BASE_SHAPE =
  ...
  return (
    <Link
      ...
      onMouseLeave={() => {
        ...
        tweenRef.current = gsap.to(pathRef.current, {
          keyframes: [
            { morphSVG: { shape: BASE_SHAPE }, duration: 0.5, ease: "back.in" },
          ],
        });
      }}
