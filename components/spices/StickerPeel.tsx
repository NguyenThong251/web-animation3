...
/**
 * Scroll-peeling sticker (1:1 rebuild of the original site's StickerPeel —
 * module 80308 in the original client bundle).
 *
 * A scrubbed ScrollTrigger tweens `--peel-amount` 1 -> 0 which drives the
 * clip-path peel in CSS; SVG point lights follow the cursor.
 */
export default function StickerPeel({
  ...
  useEffect(() => {
    ...
    tl.to(state, {
      ...
      ease: "none",
      onUpdate: () => {
        root.style.setProperty("--peel-amount", String(state.peel));
      },
    });
    return () => {
      tl.kill();
